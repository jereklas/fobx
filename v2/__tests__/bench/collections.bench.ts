...
Deno.bench("v2", { group: "map-get", baseline: true }, () => {
  const m = map(new Map([["a", 1], ["b", 2], ["c", 3]]))
  m.get("a")
  m.get("b")
  m.get("c")
})
...
Deno.bench("v2", { group: "map-has", baseline: true }, () => {
  const m = map(new Map<string, number>([["a", 1], ["b", 2]]))
  m.has("a")
  m.has("b")
  m.has("c")
})
...
Deno.bench("v2", { group: "map-delete", baseline: true }, () => {
  const m = map(new Map<string, number>([["a", 1], ["b", 2], ["c", 3]]))
  m.delete("a")
  m.delete("b")
})
