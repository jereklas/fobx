import type { IObservableObjectAdmin } from "../observables/observableObject.ts"
import type { IObservableCollectionAdmin } from "../observables/observable.ts"
import type { IComputedAdmin } from "../reactions/computed.ts"
...
export const isPlainObject = (obj: unknown) => {
  ...
  const protoCtor = Object.hasOwnProperty.call(proto, CTOR_STR) &&
    proto.constructor
...
const isObservableObjectAdmin = (
  obj: unknown,
): obj is IObservableObjectAdmin => {
  if (!isObject(obj)) return false
...
export const isComputed = (obj: unknown, prop?: PropertyKey): boolean => {
  ...
  if (!admin) return false
  return prop
    ? admin.values !== undefined && admin.values.has(prop) &&
      isComputedValueAdmin(admin.values.get(prop)[$fobx])
...
export const isObservableArray = (arr: unknown): boolean => {
  return Array.isArray(arr) &&
    arr[$fobx as unknown as keyof typeof arr] !== undefined
}

export const isMap = (thing: unknown): thing is Map<Any, Any> => {
  return thing != null &&
    Object.prototype.toString.call(thing) === "[object Map]"
}
...
export const isSet = (thing: unknown): thing is Set<Any> => {
  return thing != null &&
    Object.prototype.toString.call(thing) === "[object Set]"
}
/**
...
export const isAction = (value: unknown): boolean => {
  return typeof value === "function" &&
    value[$fobx as unknown as keyof typeof value] === "action"
}

export const isFlow = (value: unknown): boolean => {
  return typeof value === "function" &&
    value[$fobx as unknown as keyof typeof value] === "flow"
}
 ...
 */
export const isObservableCollection = (
  obj: unknown,
): obj is { [$fobx]: IObservableCollectionAdmin } => {
  if (!isObject(obj)) return false
  const admin = obj[$fobx]
  if (!admin) return false
  return isObservableCollectionAdmin(admin)
}
const isObservableCollectionAdmin = (
  obj: unknown,
): obj is IObservableCollectionAdmin => {
  if (!isObject(obj)) return false
  return obj.changes !== undefined && obj.previous !== undefined &&
 ...
 */
export const isDecoratorContext = (
  context: unknown,
): context is DecoratorContext => {
...
export const isGenerator = (value: unknown) => {
  return typeof value === "function" &&
    (value as Any)[Symbol.toStringTag] === "GeneratorFunction"
}
