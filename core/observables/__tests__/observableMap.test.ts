...
import { beforeEach, expect, fn, grabConsole, test } from "@fobx/testing"
...
test("has should not throw on invalid keys", function () {
  const x = fobx.observable(new Map())
...
test("map modifier with modifier", () => {
  ...
  const y = fobx.observable({
    a: fobx.observable(new Map([["b", {}]]), { shallow: true }),
  })
  expect(fobx.isObservableObject(y)).toBe(true)
  expect(fobx.isObservableMap(y.a)).toBe(true)
...
test(".replace should not breaks entities order!", () => {
...
test(".replace should invoke autorun", () => {
  const original = fobx.observable(
    new Map([
      ["a", "a"],
      ["b", "b"],
...
test(".replace should not report changed unnecessarily", () => {
  const mapArray: [string, string][] = [
    ["swappedA", "swappedA"],
    ["swappedB", "swappedB"],
    ["removed", "removed"],
  ]
  const replacementArray: [string, string][] = [mapArray[1], mapArray[0], [
    "added",
    "added",
  ]]
...
test("cannot replace maps anymore", () => {
...
test("Map should not be inherited", () => {
  ...
  expect(() => {
    ...
    "[@fobx/core] Cannot make observable map from class that inherit from Map: MyMap",
  )
})
...
test("subscribe to not yet existing map keys", async () => {
