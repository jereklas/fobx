...
describe("createTracker", () => {
  test("tracks dependencies and fires onInvalidate when they change", () => {
    const invalidate = fn()
    const tracker = fobx.createTracker(invalidate)
  ...
  test("suppresses run when withoutTracking + runInTransaction is used during track", () => {
    const invalidate = fn()
    const tracker = fobx.createTracker(invalidate)
    ...
    // props inside withoutTracking + runInTransaction
  ...
  test("works with computed values", () => {
    ...
    const firstName = fobx.box("John")
    const lastName = fobx.box("Doe")
  ...
  test("nested trackers work independently", () => {
    ...
    const tracker1 = fobx.createTracker(invalidate1)
    const tracker2 = fobx.createTracker(invalidate2)
