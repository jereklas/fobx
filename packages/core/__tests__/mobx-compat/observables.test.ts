...
test("basic", function () {
  ...
  expect(getGlobalState().isRunningReactions).toBe(false);
});
...
test("computed with asStructure modifier", function () {
  ...
  expect(b).toStrictEqual([{ sum: 8 }, { sum: 9 }]);
...
test("readme1", function () {
  ...
  expect(b).toStrictEqual([24]);
  order.price.value = 10;
  expect(b).toStrictEqual([24, 12]);
  expect(getGlobalState().isRunningReactions).toBe(false);
});
...
test("multiple view dependencies", function () {
  ...
  const dis = fobx.autorun(function () {
    fCalcs++;
    if (zwitch) buffer.push(b.value + d.value);
...
test("computed values believe NaN === NaN", function () {
  ...
  expect(buf).toStrictEqual(["NaN", "6"]);
});
